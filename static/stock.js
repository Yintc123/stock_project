...
async function init(){
    ...
    spec_data = await stock.get_stock_specific_data(stk_id, "PER");
    ...
    chart_parameter.chart=trading_view.load_chart("Normal", transaction_data["stock_transaction"]);
    search.hide_loading();
}
...
function create_table(s_data, t_data, data_abbreviation){
    ...
    const titles=[
        ...
        {"year":"年", "stock_name":"股票名稱", "stock_id":"股票代號"},
    ];
...
button_crosshair.addEventListener("click", () => {
    ...
    if (button_crosshair.textContent=="Magnet"){
        ...
        chart_parameter.chart=trading_view.load_chart("Magnet", transaction_data["stock_transaction"]);
    }else{
...
for (let i=0;i<button_sma.length;i++){
    button_sma[i].addEventListener("click", () => {
        ...
        chart_parameter["sma_"+days]=trading_view.add_sma(chart_parameter.chart, transaction_data["stock_transaction"], days, color);
    })
}
